...
#ifdef PAR2SER_HW
static struct Interrupt rx_int;     /* cia.resource FLAG ICR vector        */
...
/* RX doorbell: the adapter pulls CIA-A /FLAG low (open-drain ACK, DB25
 * pin 10) while the FT240X RX FIFO is non-empty AND the CPLD FSM is idle
 * (drive_ack = S_IDLE & has_data). /FLAG is an interrupt-only input on the
 * CIA -- it is NOT readable as a PRA/PRB bit (netlist: CPLD pin 31 -> J1
 * pin 10 only) -- so the interrupt itself is the one and only "data
 * available" signal.
 *
 * We register on the FLAG ICR bit via cia.resource (AddICRVector) rather
 * than a generic INTB_PORTS server: cia.resource dispatches ONLY genuine
 * FLAG events to us and owns the ICR read/clear correctly alongside the
 * system's other CIA-A users (keyboard/timers). A raw INTB_PORTS server
 * would run on every ports interrupt and, with an unconditional read,
 * issue garbage READ1s for events that weren't ours.
 *
 * Model: exactly ONE byte per FLAG. The handler does one unconditional
 * READ1 via transport_poll_rx(). If more bytes remain in the FIFO, the
 * FSM's return to S_IDLE re-asserts ACK -> a fresh /FLAG falling edge ->
 * we are called again. The retrigger is generated by hardware, so the
 * drain is self-clocking and the handler never loops.
 *
 * Runs in interrupt context: no AllocMem; ReplyMsg from here is fine. */
static ULONG __attribute__((used)) rx_flag_handler(APTR data asm("a1"))
{
    UBYTE c;
    (void)data;
    if (transport_poll_rx(&c))
        rx_feed(c);
    service_reads();
    return 0;
}

static struct Library *ciaabase;   /* cia.resource (CIA-A) */

static BOOL __attribute__((used)) rx_install(void)
{
    if (rx_int_added)
        return TRUE;

    ciaabase = (struct Library *)OpenResource((CONST_STRPTR)CIAANAME);
    if (!ciaabase) {
        DBG("  rx_install: no ciaa.resource\n");
        return FALSE;
    }
    ...
    rx_int.is_Data = NULL;
    rx_int.is_Code = (void (*)())rx_flag_handler;

    Disable();
    if (AddICRVector(ciaabase, CIAICRB_FLG, &rx_int)) {
        /* non-NULL return = FLAG vector already owned by someone else */
        Enable();
        DBG("  rx_install: CIA-A FLAG vector busy\n");
        return FALSE;
    }
    /* Clear any stale latched FLAG, then enable the FLAG interrupt. */
    SetICR(ciaabase, CIAICRF_FLG);
    AbleICR(ciaabase, CIAICRF_SETCLR | CIAICRF_FLG);
    Enable();

    rx_int_added = TRUE;
    DBG("  rx_install: FLAG ICR vector installed\n");
    return TRUE;
}
...
static void __attribute__((used)) rx_remove(void)
{
    if (!rx_int_added)
        return;
    AbleICR(ciaabase, CIAICRF_FLG);              /* disable FLAG int */
    RemICRVector(ciaabase, CIAICRB_FLG, &rx_int);
    rx_int_added = FALSE;
}
#endif /* PAR2SER_HW */
                    ...
                    ULONG unitnum, ULONG flags)
{
    ...
    if (!is_open) {
        ...
        if (!rx_install()) {
            DBG("  reject: rx_install failed (FLAG vector busy?)\n");
            transport_shutdown();
            FreeMem(rbuf, rbuf_len); rbuf = NULL; rbuf_len = 0;
            return;   /* io_Error stays IOERR_OPENFAIL (set at entry) */
        }
        /* Generate the doorbell edge for any data that arrived before the
         * FLAG interrupt was enabled (CIA latches edges, not levels). */
        transport_rx_prime();
#else
        DBG("  RX gated off (PAR2SER_RX_ENABLED=0): no FLAG vector\n");
#endif
#endif
        is_open = TRUE;
    }
